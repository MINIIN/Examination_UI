...
export class InMemDataService implements InMemoryDbService {
  ...
  get(reqInfo: RequestInfo) {
    ...
    if (is(reqInfo, 'me/menu')) {
      ...
      if (permisson == '1') {
        return ajax('assets/data/menu1.json?_t=' + Date.now()).pipe(
          map((response: any) => {
      ...
      if (permisson == '2') {
        return ajax('assets/data/menu2.json?_t=' + Date.now()).pipe(
          map((response: any) => {
            return { headers, url, status: STATUS.OK, body: { menu: response.response.menu } };
          }),
          switchMap(response => reqInfo.utils.createResponse$(() => response))
      ...
      if (permisson == '3') {
        return ajax('assets/data/menu3.json?_t=' + Date.now()).pipe(
          map((response: any) => {
            return { headers, url, status: STATUS.OK, body: { menu: response.response.menu } };
          }),
          switchMap(response => reqInfo.utils.createResponse$(() => response))
